"""
Quality Assurance for automated reduction of P200 NGPS.
"""
...
def save_2dspecs(qa_dict: dict, output_spec2ds: List[str], output_path: str,
    ...
    arm = 'R' if 'R' in spectrograph else 'I'
    ...
    for fname in output_spec2ds:
        ...
        ut = spec.head0['SHUTTIME']
        fname = spec.head0['FILENAME']

        if obj_png_dict.get(spec.head0['TARGET']):
            obj_png_dict[spec.head0['TARGET']]['pngs'].append(outname)
            obj_png_dict[spec.head0['TARGET']]['airmass'].append(airmass)
            obj_png_dict[spec.head0['TARGET']]['time'].append(ut)
            obj_png_dict[spec.head0['TARGET']]['fname'].append(fname)
        else:
            obj_png_dict[spec.head0['TARGET']] = {
                'pngs': [outname],
                'airmass': [airmass],
                'time': [ut],
                'fname': [fname]
...
def write_extraction_QA(qa_dict: dict, output_path: str) -> None:
    ...
    with tag('html'):
        with tag('head'):
            with tag('title'):
                text("NGPS Extraction QA")
            doc.stag('link', rel='stylesheet', href='ngps_qa.css')
            with tag('script', src='./ngps_qa.js'):
                pass
        with tag('body'):
            with tag('div'):
                doc.attr(klass='tab')
    ...
    shutil.copyfile(resource_filename("ngps_drp", "data/ngps_qa.js"), os.path.join(out_path, "ngps_qa.js"))
    shutil.copyfile(resource_filename("ngps_drp", "data/ngps_qa.css"), os.path.join(out_path, "ngps_qa.css"))
