...
from ngps_pipe import splicing, adjust_splicing

def entrypoint():
    main(parse())

def parse(options: Optional[List[str]] = None) -> argparse.Namespace:
    argparser = argparse.ArgumentParser(description="Manually splice two coadded files together for NGPS.\n"
        "After preparing the spliced spectrum, the ngps_adjust_splicing GUI pops up to allow for the splicing "
