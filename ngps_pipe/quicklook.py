...
from ngps_pipe import show_spectrum
...
def parse(options: Optional[List[str]] = None) -> argparse.Namespace:
    argparser = argparse.ArgumentParser(description="Quicklook for P200 NGPS",
...
def main(args: argparse.Namespace):
    ...
    if all('I' in os.path.basename(fname) for fname in args.fname):
        spectrograph = 'p200_ngps_I'
    elif all('R' in os.path.basename(fname) for fname in args.fname):
    ...
    pypeIt = pypeit.PypeIt(ofiles[0], verbosity=0,
                           ...
                           logname='ngps_ql.log', show=False, calib_only=calib_only)
    if calib_only:
        pypeIt.calib_all()
    else:
        pypeIt.reduce_all()
    ...
    if output_spec1ds and not calib_only:
        ...
        sensfiles = [resource_filename("ngps_drp", f"data/sens_{config}.fits")]
