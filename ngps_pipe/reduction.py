"""
Automated reduction steps for P200 NGPS.
"""
...
# TEMPORARY WHILE USING CUSTOM PYPEIT VERSION
import sys
...
from ngps_pipe.manual_tracing import ManualTracingGUI
...
def set_calibs(table):
    """
    Automatically set 'calib' column for .pypeit file.

    flat, trace, pixelflat, and illumflat frames get unique calib IDs based on their slitwid values.
    Frames with similar slitwid values (within 0.01 difference) are grouped together.
    Each unique slitwid gets a unique calib ID starting from 0.

    Args:
        table (list of dicts): PypeItSetup.fitstbl.table as a list of dictionaries
    """
    ...
    calib_ID = 0
    max_slitwid_diff = 0.01 
    slitwid_to_calib = []  

    for row in table:
        if 'trace' in row['frametype'] or 'pixelflat' in row['frametype'] or 'illumflat' in row['frametype'] or 'science' in row['frametype'] or 'standard' in row['frametype']:
            slitwid = row['slitwid']

            # Check if slitwid is within the threshold of any existing slitwid
            assigned = False
            for existing_slitwid, assigned_calib in slitwid_to_calib:
                if abs(existing_slitwid - slitwid) < max_slitwid_diff:
                    row['calib'] = assigned_calib
                    assigned = True
                    break

            # If not assigned, assign a new calib ID and add it to the list
            if not assigned:
                row['calib'] = calib_ID
                slitwid_to_calib.append((slitwid, calib_ID))
                calib_ID += 1  # Increment calib ID for next unique slitwid


def set_comb_ids(table):
    """
    Automatically set 'comb_id' column for .pypeit file by grouping science and standard frames 
    with the same target name and the same calibration group.
    ...
    # Iterate through the rows to process science and standard frames
    for row in table:
        if 'science' in row['frametype'] or 'standard' in row['frametype']:
            target_name = row['target']  
            calib = row['calib'] 

            if (target_name, calib) not in target_comb_groups:
                target_comb_groups[(target_name, calib)] = comb_id
                row['comb_id'] = comb_id
                comb_id += 1  # Increment comb_id for the next group
            else:
                # If found, assign the same comb_id to this group
                row['comb_id'] = target_comb_groups[(target_name, calib)]
        else:    
            # Calibration frames get a comb_id of '-1'
            row['comb_id'] = -1
...
def write_setup(context: Tuple[PypeItSetup, str], cfg_split: str,
    ...
    # NGPS_PIPE user, so PypeIt's defaults are good enough AFTER frametypes are
    # corrected.
    ...
    # ps.user_cfg.append('master_dir = Master_' + spectrograph.split('_')[-1]) # Not an accepted parameter, find new way to do this

    #ps.user_cfg.append('[baseprocess]')
    #ps.user_cfg.append('use_pixelflat = False')
    #ps.user_cfg.append('use_illumflat = False')
...
def redux(pypeit_file: str, output_path: str, reuse_masters: bool = True,
    ...
    # Don't re-run pipeline for now to speed up testing, just grab existing reduction. ##################################

    pypeIt = pypeit.pypeit.PypeIt(pypeit_file, verbosity=2,
                           overwrite=True,
                           redux_path=output_path, 
...
def manual_extraction_GUI(output_spec2ds: List[str], output_path: str) -> dict:
    ...
    print("Preparing Manual Extraction") 
    gui_dict = {}
    for spec2d in spec2ds:
        path = os.path.join(output_path, 'Science', spec2d)
        # open fits file
        spec = Spec2DObj.from_file(path, 'DET01')
        ...
        base_name = '_'.join(os.path.basename(path).split('_')[3:-1])
        ...
        # Fixed: base_name was always ngps and was being overriden with this key for every 2d spectrum
        gui_dict[base_name] = {
            'spec': spec,
            'edges': edges,
            'traces': traces,
...
def manual_extraction(output_spec2ds: List[str], pypeit_file: str,
    ...
    manual_lines_fn = lambda targ: ['# Added by NGPS_PIPE for manual extraction\n',
            '[reduce]\n',
            '[[extraction]]\n',
            'use_user_fwhm = True\n',
            '[[[manual]]]\n',
